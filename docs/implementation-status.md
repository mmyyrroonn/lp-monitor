...
状态：H1–H4 代码与离线验证均未实施，H5 实链样本未运行；本轮仅文档。下一窗口收到实施指令后按计划顺序执行。此前的“历史缺失能力暂不补”对本次明确的有限热度研究范围不再适用；不扩展为整链归档或长期无人值守采集。P5/P6 既有实测状态保持不变。

计划基线为 `main@c30048f`，包含默认 follow 从最新块启动、历史路径目录不完整会退出、原生回放快照缺口、旧 minute-close 与当前 rolling 口径不同等已核查事实。具体代码/证据路径与预算假设见设计；执行前重新核对当前 checkout。
