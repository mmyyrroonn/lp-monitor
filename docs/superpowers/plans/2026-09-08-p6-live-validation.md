...
- [ ] 从观察时段导出含分钟索引和批次时间的manifest，用recorded-observed离线replay对比窗口/告警；可见真实reorg则保存，未发生不能声称实链reorg验证，只能说故障fixture通过。

## Task 6.3：成本与下一窗口交接

**Files:** 修改docs/runbook.md、docs/implementation-status.md、START_HERE.md。

- [ ] 运行手册给出probe、follow、status、停止、恢复、backfill、replay、升级前备份的准确命令。数据备份用SQLite backup API，不只复制运行中的主文件漏掉WAL。
...
P0–P6工程门槛通过即可交付“本机只读机会监控器”。历史未支持的收益结论保持未知。
