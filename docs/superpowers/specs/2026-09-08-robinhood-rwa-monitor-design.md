...
2026-09-09优先级修订：P0–P4现已验收，当前实现以入口和实施状态为准。用户将历史数据获取降为低优先级：已有能力/数据保留，缺失获取功能及专门研究回填暂缓；优先实时采集、热度与本机提醒稳定性。P5完整历史评估不再是P6前置条件，以下历史设计保留为按需扩展。阈值可由用户额外分析提供，未独立评估的效果保持未知。实时启动、发现与恢复必需的既有扫描继续保留。

2026-09-10实施更新：用户安排实现P5离线回放、完整队列与阈值比较；不增加历史采集，代码验收与历史完整性/策略有效性分别记录。当前结果见 [P5报告](../../../artifacts/p5/report.md)，P6仍未启动。
...
阶段映射：G1/G2/G6/G7=P0；范围记录/时间索引/修正=P1；协议解码与观测=P2；分钟热度=P3；提醒与再热=P4；历史热度评估=P5；本机运行=P6。按2026-09-09用户修订，首版实时监控交付以P0–P4及P6适用工程验收为准，P5可保持pending；缺失的P5回放对照须披露为暂缓。阈值效果需独立证据，不能由运行验收推断。
