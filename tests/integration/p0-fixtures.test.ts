...
import { sha256, resolveEvidencePath } from '../../src/ops/files.js';
...
const runs = existsSync(root)
  ? readdirSync(root).filter((name) => existsSync(join(root, name, 'manifest.json')))
  : [];
test.skipIf(!runs.length)(
  'captured real RPC logs retain source hashes and decode independently',
  () => {
    ...
    for (const run of runs) {
      ...
      const logs: RawLog[] = JSON.parse(readFileSync(join(root, run, 'logs.json'), 'utf8')).map(
        (log: RawLog) => ({ ...log, blockNumber: BigInt(log.blockNumber) }),
      );
      ...
      for (const log of logs) {
        ...
        const protocol = log.address.toLowerCase() === config.v4Manager.toLowerCase() ? 'v4' : 'v3';
        expect(
          interfaces[protocol].parseLog({ topics: [...log.topics], data: log.data }),
        ).not.toBeNull();
      }
