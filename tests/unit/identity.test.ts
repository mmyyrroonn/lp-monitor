...
const anchor: BlockAnchor = { number: 10_000n, hash: `0x${'ab'.repeat(32)}`, timestampSec: 1_788_500_000 };
const stateViewAbi = parseAbi(['function poolManager() view returns (address)']);
const code = `0x${'60'.repeat(20)}` as Hex;

type FixtureOptions = { chainId?: number; deployedAt?: bigint; history?: 'available' | 'unsupported'; stateViewCall?: 'available' | 'unsupported'; poolFactory?: Address; changedAnchor?: boolean; maxCalls?: number };
  ...
  return {
    ...
    async request(method, params) {
      ...
      if (method === 'eth_getCode') {
        ...
        if (options.history === 'unsupported' && block !== anchor.number) throw new Error('historical state unavailable at https://provider.invalid/?key=secret');
      ...
      if (call.to.toLowerCase() === config.stateView.toLowerCase()) {
        ...
        return encodeFunctionResult({ abi: stateViewAbi, functionName: 'poolManager', result: config.v4Manager });
      }
      if (Object.values(config.tokens).some((address) => address.toLowerCase() === call.to.toLowerCase())) {
      ...
      if (call.to.toLowerCase() === config.v3Pools[0]!.toLowerCase()) {
        ...
        if (decoded.functionName === 'factory') return encodeFunctionResult({ abi: v3PoolAbi, functionName: 'factory', result: options.poolFactory ?? config.v3Factory });
        if (decoded.functionName === 'token0') return encodeFunctionResult({ abi: v3PoolAbi, functionName: 'token0', result: config.tokens.AMC });
        if (decoded.functionName === 'token1') return encodeFunctionResult({ abi: v3PoolAbi, functionName: 'token1', result: config.tokens.USDG });
        if (decoded.functionName === 'fee') return encodeFunctionResult({ abi: v3PoolAbi, functionName: 'fee', result: 3_000 });
      }
      if (call.to.toLowerCase() === config.v3Factory.toLowerCase()) {
        return encodeFunctionResult({ abi: v3FactoryAbi, functionName: 'getPool', result: config.v3Pools[0]! });
      }
...
test('verifies fixed-anchor code and callable identities, then proves deployment block 9070', async () => {
  ...
  expect(report.v3Pools[0]).toMatchObject({ factory: config.v3Factory, token0: config.tokens.AMC, token1: config.tokens.USDG, fee: 3_000, status: 'verified' });
  expect(report.deployments.v3Factory).toMatchObject({ status: 'verified', firstCodeBlock: 9_070n, candidateChecked: true });
  expect(report.deployments.v4Manager).toMatchObject({ status: 'verified', firstCodeBlock: 9_070n, candidateChecked: true });
...
test('keeps current identity distinct from unavailable historical deployment proof', async () => {
  ...
  expect(report.status).toBe('unverified');
  expect(report.requiredPassed).toBe(true);
  expect(report.deployments.v3Factory).toMatchObject({ status: 'unverified', firstCodeBlock: null, candidateChecked: true });
  expect(report.deployments.v3Factory.reason).toBe('RPC historical-state-missing');
  expect(Object.values(report.deployments).map(({ reason }) => reason).join(' ')).not.toContain('secret');
});
...
test('treats candidate 9070 as a hint and finds a later deployment', async () => {
  ...
  expect(report.deployments.v3Factory).toMatchObject({ status: 'verified', firstCodeBlock: 9_500n, candidateChecked: true });
  expect(report.deployments.v4Manager).toMatchObject({ status: 'verified', firstCodeBlock: 9_500n, candidateChecked: true });
  expect(report.requiredPassed).toBe(true);
});

test('rejects evidence when the fixed anchor hash changes during verification', async () => {
  await expect(verifyIdentity(fixtureReader({ changedAnchor: true }), config, anchor)).rejects.toThrow(/anchor/i);
});
...
test('treats StateView.poolManager as optional but records its failure', async () => {
  const report = await verifyIdentity(fixtureReader({ stateViewCall: 'unsupported' }), config, anchor);

  expect(report.currentIdentityStatus).toBe('verified');
  expect(report.contracts.stateView.status).toBe('verified');
  expect(report.stateViewPoolManager).toMatchObject({ status: 'unverified', required: false, observed: null });
});

test('rejects a V3 pool whose own factory differs from the configured factory', async () => {
  const report = await verifyIdentity(fixtureReader({ poolFactory: '0x0000000000000000000000000000000000000001' }), config, anchor);
