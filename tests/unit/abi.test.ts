...
describe('official Uniswap ABI boundary', () => {
  test('computes the V4 PoolId with the official ABI encoding and SDK cross-check', () => {
    ...
    expect(computeV4PoolId(key)).toBe(Pool.getPoolId(token0, token1, key.fee, key.tickSpacing, key.hooks));
  });

  test('rejects a V4 PoolKey whose currencies are not strictly sorted', () => {
    expect(() => computeV4PoolId({ ...key, currency0: currency1, currency1: currency0 })).toThrow(/sorted/i);
    expect(() => computeV4PoolId({ ...key, currency1: currency0 })).toThrow(/sorted/i);
  });

  test('matches official V3 event signatures', () => {
    const poolCreated = v3FactoryAbi.find((item) => item.type === 'event' && item.name === 'PoolCreated');
    const swap = v3PoolAbi.find((item) => item.type === 'event' && item.name === 'Swap');

    expect(poolCreated).toBeDefined();
    expect(swap).toBeDefined();
    expect(toEventSelector(poolCreated!)).toBe('0x783cca1c0412dd0d695e784568c96da2e9c22ff989357a2e8b1d9b2b4e6b7118');
    expect(toEventSelector(swap!)).toBe('0xc42079f94a6350d7e6235f29174924f928cc2ac818eb64fed8004e115fbcca67');
  });

  test('returns an explicit error for unknown V4 topics', () => {
    expect(() => decodeV4ManagerEvent({
  ...
  test('does not accept malformed data for a recognized V4 topic', () => {
    const initialize = v4ManagerAbi.find((item) => item.type === 'event' && item.name === 'Initialize');
