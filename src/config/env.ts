import { z } from 'zod';
export interface RuntimeEnv { httpRpcUrl: string; wsRpcUrl?: string; providerAlias: string; dataDir: string }
...
export function loadEnv(env: NodeJS.ProcessEnv): RuntimeEnv {
  function endpoint(key: string, protocols: string[], optional = false): string | undefined {
    ...
    try {
      ...
      return value;
    } catch { throw new ConfigError(`Invalid or missing ${key}`); }
  }
  const httpRpcUrl = endpoint('RH_RPC_HTTP', ['http:', 'https:'])!;
  const wsRpcUrl = endpoint('RH_RPC_WS', ['ws:', 'wss:'], true);
  const alias = z.string().regex(/^[a-zA-Z0-9_-]{1,64}$/).safeParse(env.RH_PROVIDER_ALIAS || 'configured-rpc');
