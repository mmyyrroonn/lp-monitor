...
export type PoolKey = { currency0: Address; currency1: Address; fee: number;
  tickSpacing: number; hooks: Address };
export type LogRef = { blockHash: Hex; blockNumber: bigint; transactionHash: Hex;
...
export type RawLog = LogRef & { address: Address; topics: readonly Hex[]; data: Hex;
  rawBlockTimestamp: Hex | null };
export type LogTime = { minuteStartSec: number | null; exactTimestampSec: number | null;
  source: 'log-verified' | 'minute-boundary' | 'unresolved' };
export type MinuteBoundary = { timestampSec: number; firstBlock: bigint;
  before: BlockAnchor; at: BlockAnchor };
export type RangeBatch = { id: string; scopeId: WatchScopeId; fromBlock: bigint; toBlock: bigint;
...
export type RangeChangeSet = { added: readonly LogRef[]; removed: readonly LogRef[];
  retimed: readonly LogRef[]; affectedFromBlock: bigint };
export type Quality = { coverage: 'complete' | 'gap' | 'warming' | 'rechecking';
  timing: 'second' | 'minute' | 'unresolved';
  valuation: 'usd-estimate' | 'usdg-only' | 'unpriced' };
export type Swap = { kind: 'swap'; ref: LogRef; time: LogTime; pool: PoolRef;
...
export type LiquidityChange = { kind: 'liquidity'; ref: LogRef; time: LogTime; pool: PoolRef;
  tickLower: number; tickUpper: number; delta: bigint; actor: Address;
  salt: Hex | null };
export type AncillaryEvent = { kind: 'initialize' | 'collect' | 'donate' | 'other';
...
export type PoolObservation = { pool: PoolRef; lastSwap: Swap | null;
  lastLiquidityAction: LiquidityChange | null };
export type QuoteObservation = { token: Address; quote: Address;
  numerator: bigint; denominator: bigint; effectiveAt: LogRef;
  time: LogTime; source: string; maxAgeSec: number };
export type Alert = { id: string; revision: number; pool: PoolRef;
...
export interface PoolDecoder { decode(log: RawLog, time: LogTime, pool: PoolRef): PoolEvent }
