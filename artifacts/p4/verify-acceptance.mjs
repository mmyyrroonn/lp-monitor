...
mkdirSync('artifacts/p4',{recursive:true});
const hash=v=>createHash('sha256').update(v).digest('hex');
const save=(name,value)=>writeFileSync('artifacts/p4/'+name,encodeJson(value)+'\n');
const sourcePath='data/p3-acceptance.sqlite',targetPath='data/p4-acceptance.sqlite';
...
const decide=(input)=>{const d=evaluateSignal(previous,input,initialSignalConfig);previous=d.nextSnapshot;demo.transaction(()=>commitSignalDecision(demo,'synthetic',d,'synthetic'))();return d;};
...
writeFileSync('artifacts/p4/alerts.jsonl','');
const sink=createJsonlSink('artifacts/p4/alerts.jsonl');
for(const a of records)await sink(a);
writeFileSync('artifacts/p4/rendered.txt','合成离线示例，不是当前行情或已验证收益。liquidity-watch 仅为格式示例，未配置自动触发规则。\n\n'+records.map(formatAlert).join('\n\n---\n\n')+'\n');
demo.close();
...
save('acceptance.json',{status:'passed',scope:'P4 offline engineering acceptance',historical,synthetic:{records:records.length,engineKinds:[...kinds],formatOnlyKinds:['liquidity-watch'],activeCooldownUpgradeSeconds:10},
