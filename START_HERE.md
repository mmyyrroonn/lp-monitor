...
## 2026-09-13 新窗口：股票池完整目录与热度研究

用户已确认先做“完整股票池目录 + 有代表性的有限历史 + 持续实时采集”，不先保存整条链。本轮仅编写计划，代码、数据迁移与新采集均未执行。

实施时先读[主计划与新窗口提示](docs/superpowers/plans/2026-09-13-stock-pool-heat-research.md)和[设计合同](docs/superpowers/specs/2026-09-13-stock-pool-heat-research-design.md)，按 **H1 目录与 RPC 恢复 → H2 存储压缩 → H3 独立历史作业 → H4 滚动热度研究** 实施代码和离线验证。H5 的真实目录/一天级基准/14–28 天研究采集由用户在执行窗口安排范围与预算；不自动改线上阈值、购买数据或启动常驻服务。

当前源码的默认 `follow` 不带 `--from-block` 时从本次最新块开始，复用已知池，不做历史 bootstrap 或停机补采；指定历史起点的路径才需要历史目录。下方旧文档中“第一次运行都要完成历史发现”的表述仅适用于历史路径，以此处和当前源码为准。新计划也不会改变默认实时启动语义。

新计划替代“历史获取一概暂缓”的旧优先级描述，但只允许为热度研究建设有限范围能力；P5 数据完整性、阈值效果与 P6 长时实测仍须分别验收，不能因计划已写好标记通过。
